...
import('./gen_science_ch11.js').then(() => {
import('./gen_math_ch11.js').then(() => {
import('./gen_english_ch11.js').then(() => {
import('./gen_gk_ch11.js').then(() => {
import('./gen_social_ch11.js').then(() => {
import('./gen_hindi_ch11.js').then(() => {
  const sci = JSON.parse(fs.readFileSync(path.join(dir, 'science_ch11.json'), 'utf8'));
  const mat = JSON.parse(fs.readFileSync(path.join(dir, 'math_ch11.json'), 'utf8'));
  const eng = JSON.parse(fs.readFileSync(path.join(dir, 'english_ch11.json'), 'utf8'));
  const gk  = JSON.parse(fs.readFileSync(path.join(dir, 'gk_ch11.json'), 'utf8'));
  const soc = JSON.parse(fs.readFileSync(path.join(dir, 'social_ch11.json'), 'utf8'));
  const hin = JSON.parse(fs.readFileSync(path.join(dir, 'hindi_ch11.json'), 'utf8'));

  const allQuestions = [...sci, ...mat, ...eng, ...gk, ...soc, ...hin];

  console.log('Total Assembled Questions in Batch 11:', allQuestions.length);
  ...
  allQuestions.forEach((q, idx) => {
    if (q.classLevel !== 6) errors.push(`[${idx}] Invalid classLevel`);
  ...
  console.log('\n📊 Batch 11 Subject & Difficulty Breakdown:');
  ...
  if (errors.length === 0 && allQuestions.length === 240) {
    const finalFile = path.join(dir, 'batch_11.json');
