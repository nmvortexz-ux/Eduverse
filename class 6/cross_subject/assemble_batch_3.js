...
import('./gen_science_ch3.js').then(() => {
import('./gen_math_ch3.js').then(() => {
import('./gen_english_ch3.js').then(() => {
import('./gen_gk_ch3.js').then(() => {
import('./gen_social_ch3.js').then(() => {
import('./gen_hindi_ch3.js').then(() => {
  const sci = JSON.parse(fs.readFileSync(path.join(dir, 'science_ch3.json'), 'utf8'));
  const mat = JSON.parse(fs.readFileSync(path.join(dir, 'math_ch3.json'), 'utf8'));
  const eng = JSON.parse(fs.readFileSync(path.join(dir, 'english_ch3.json'), 'utf8'));
  const gk  = JSON.parse(fs.readFileSync(path.join(dir, 'gk_ch3.json'), 'utf8'));
  const soc = JSON.parse(fs.readFileSync(path.join(dir, 'social_ch3.json'), 'utf8'));
  const hin = JSON.parse(fs.readFileSync(path.join(dir, 'hindi_ch3.json'), 'utf8'));

  const allQuestions = [...sci, ...mat, ...eng, ...gk, ...soc, ...hin];

  console.log('Total Assembled Questions in Batch 3:', allQuestions.length);
  ...
  console.log('\n📊 Batch 3 Subject & Difficulty Breakdown:');
  ...
  if (errors.length === 0 && allQuestions.length === 240) {
    const finalFile = path.join(dir, 'batch_3.json');
